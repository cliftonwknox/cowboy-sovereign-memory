...
- You can run shell commands on this host and write to the Hermes home (`~/.hermes/`).
...
own and recalls by meaning. It runs entirely on your machine. Here's the quick check:
  ...
  Footprint:      one local database file in ~/.hermes/iconic/ — nothing leaves your machine
...
```
python3 -m venv ~/.hermes/iconic/venv
~/.hermes/iconic/venv/bin/pip install -U pip
~/.hermes/iconic/venv/bin/pip install fastembed numpy hermes-iconic-memory   # or: pip install <ICONIC_SOURCE>
```
**Verify:** `~/.hermes/iconic/venv/bin/python -c "import fastembed, numpy, hermes_iconic_memory; print('ok')"` → prints `ok`.

### 3.2 Embedder warm-up (pre-fetch so first recall isn't slow)
```
~/.hermes/iconic/venv/bin/python -m hermes_iconic_memory.warm_embedder
```
...
```
~/.hermes/iconic/venv/bin/pip install llama-cpp-python
# download gemma-2-2b-it Q4_K_M GGUF (accept the Gemma license on the source first):
mkdir -p ~/.hermes/iconic/models
# place: ~/.hermes/iconic/models/gemma-2-2b-it-Q4_K_M.gguf  (~1.7 GB)
```
**Verify:** the GGUF file exists and is ~1.6–1.8 GB; `~/.hermes/iconic/venv/bin/python -m
hermes_iconic_memory.check_dreamer --model ~/.hermes/iconic/models/gemma-2-2b-it-Q4_K_M.gguf` loads it
on CPU and prints a one-line test completion.

### 3.4 Install the plugin into Hermes
```
mkdir -p ~/.hermes/plugins/memory/hermes-iconic-memory
~/.hermes/iconic/venv/bin/python -m hermes_iconic_memory.install_plugin --hermes-home ~/.hermes
```
**Verify:** `hermes plugins list` shows `hermes-iconic-memory` as available.

### 3.5 Configuration (explain each choice to the human as you set it)
Config lives at `~/.hermes/iconic/config.yaml`. Generate a default, then confirm the key fields:
```
~/.hermes/iconic/venv/bin/python -m hermes_iconic_memory.init_config --mode <full|recall-only>
```
...
| `db_path` | `~/.hermes/iconic/memory.db` | where memory lives; back this up. |
...
**Verify:** `~/.hermes/iconic/venv/bin/python -m hermes_iconic_memory.doctor` → all green.
...
```
cp winnow-SKILL.md ~/.hermes/skills/winnow/SKILL.md
```
...
**Reinforcement actually raises a salience.** Recall something, run sleep, and confirm the salience
of a recalled memory went *up*:
```
...
cause — and retention is running as a blind age clock. Fix that before enabling last-use archiving.

**Last use is being recorded before retention is switched on.** Ask the store what a retention pass
would take right now:
```
hermes iconic sleep --dry-run
```
**Verify:** it reports a small number, not the whole store. A store that would archive nearly
everything is telling you the last-use column was never written, so every memory has fallen back to
its creation date. **Baseline the column first**, then enable the rule. Do not widen the retention
window to make the warning go away.

### 3.10 (Optional) schedule maintenance
```
# cron the nightly sleep (full mode):
(crontab -l 2>/dev/null; echo "0 4 * * * ~/.hermes/iconic/venv/bin/python -m hermes_iconic_memory.sleep") | crontab -
```
...
- **Uninstall is clean:** disable the plugin (`hermes plugins disable hermes-iconic-memory`) and
  remove `~/.hermes/plugins/memory/hermes-iconic-memory/`. Hermes' native memory is untouched.
